  ...
  return (
    <div className={`${SmainLayoutMargin}`}>
      <div className="container mx-auto p-6">
        <h1 className="text-4xl font-bold mb-6 text-center text-black">Order Summary</h1>
        ...
        <div className="bg-white shadow-lg rounded-lg p-6 space-y-6">
              ...
              {cart.map(item => (
                ...
                >
                  <div className="flex flex-col w-full sm:w-auto">
                    <h3 className="text-lg font-medium text-gray-700">{item.product.name}</h3>
                    <p className="text-gray-500">Unit Price: ₱ {item.product.price.toLocaleString()}</p>
                  </div>



                  <div className="flex justify-between gap-x-5 items-center w-full sm:w-auto md:w-auto">
                    <p className="font-medium text-gray-900 text-right md:text-left">₱ {(item.product.price * item.quantity).toLocaleString()}</p>
                    <div className="flex items-center space-x-2 my-2 sm:my-0 md:flex-row">
                    <button
                      onClick={() => handleChangeQuantity(item.product.id, item.quantity - 1)}
          ...
          <div className="flex justify-between items-center mt-6">
            ...
            <p className="text-xl font-semibold text-gray-900">₱ {calculateTotal().toLocaleString()}</p>
          </div>
