...
const CClientCardContainer = (props: { data: IProductCard[] }) => {
  ...
  const [selectedProduct, setSelectedProduct] = useState<IProductCard | null>(null);
    ...
    <>
        ...
        return (
          <div key={x.id} className="border pb-2 border-gray-300 rounded-lg">
            ...
            <div className="flex items-center flex-col justify-center gap-y-1">
              <h3 className="text-xl font-medium text-center text-black">{x.title}</h3>
              <p className="text-lg font-semibold text-yellow-700 text-center">{x.price}</p>
      ...
      {modalOpen && selectedProduct && (
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white p-6 rounded-lg max-w-xl w-full mx-4 md:mx-0 overflow-y-auto" style={{ maxHeight: "80vh" }}>
            <button
              onClick={closeModal}
            ...
            <div className="flex flex-col gap-4">
              ...
              <div className="flex items-center justify-center gap-4">
                {selectedProduct.subImage && selectedProduct.subImage.slice(0, 3).map((image: string, index) => (
                  <img
                    key={index}
                    src={image}
                    alt={`product-image-${index}`}
                    className={`w-24 h-24 object-cover rounded-md cursor-pointer ${selectedProduct.image === image ? "border-2 border-blue-500" : ""}`}
                    onClick={() => setSelectedProduct(prev => prev ? { ...prev, image } : prev)} // Update the main image
                  />
                ))}
              </div>
                <h3 className="text-xl font-medium text-center text-black mb-2">{selectedProduct.title}</h3>
                <p className="text-lg font-semibold text-yellow-700 text-center mb-4">{selectedProduct.price}</p>
                <p className="text-gray-600 text-center mb-4">{selectedProduct.description}</p>
                <p className="text-gray-600 text-center mb-4">Items left: {selectedProduct.quantity}</p>
                {/* Quantity Selector */}
